...
function trackTaskProgress(taskId, onUpdateCallback) {
    ...
    eventSource.onmessage = (event) => {
        ...
        progressBarFill.style.width = `${task.progress || 0}%`;
        taskProgressPercent.innerText = `${task.progress || 0}%`;
...
document.getElementById("btn-modal-recap").addEventListener("click", () => modalRecap.style.display = "flex");
document.getElementById("btn-close-modal-recap").addEventListener("click", () => modalRecap.style.display = "none");

document.getElementById("btn-call-recap-gemini").addEventListener("click", async () => {
    const content = document.getElementById("txt-recap-input").value.trim();
    const apiKey = document.getElementById("txt-recap-key").value.trim();
    if (!content) {
        alert("សូមបញ្ចូលអត្ថបទសាច់រឿង!");
        return;
    }

    taskStatusLabel.innerText = "Gemini កំពុងសម្រាយសាច់រឿង & ជ្រើសរើស Highlight...";
    startTimer();
    ...
    try {
        const res = await fetch("/api/gemini-recap", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ content, api_key: apiKey || null })
        });
        const data = await res.json();
        stopTimer();
        if (data.success) {
            ...
            taskStatusLabel.innerText = `✅ សម្រាយរឿងបាន ${subtitles.length} ឈុត Highlight!`;
        }
    } catch (err) {
        stopTimer();
        alert("កំហុស Gemini Recap: " + err);
    }
});
...
document.getElementById("btn-modal-gemini").addEventListener("click", () => modalGemini.style.display = "flex");
...
document.getElementById("btn-call-gemini").addEventListener("click", async () => {
    const content = document.getElementById("txt-gemini-input").value.trim();
    const apiKey = document.getElementById("txt-gemini-key").value.trim();
    if (!content) return;

    taskStatusLabel.innerText = "Gemini កំពុងបកប្រែ Subtitle...";
    startTimer();
    try {
        ...
        if (data.success) {
            ...
            renderSubtitlesTable();
            modalGemini.style.display = "none";
            taskStatusLabel.innerText = "បកប្រែ SRT ជាមួយ Gemini រួចរាល់!";
        }
    } catch (err) {
        stopTimer();
        alert("កំហុស Gemini AI: " + err);
    }
});
